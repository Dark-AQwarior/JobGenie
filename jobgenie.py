...
def fetch_jobs(search_term, location="USA", results_wanted=50, hours_old=24):
    ...
    try:
        ...
        ].copy()  # 🔥 Ensuring it's an independent DataFrame (important!)

        if filtered_jobs.empty:
            logging.warning(f"⚠️ No suitable jobs found for {search_term} after filtering.")
            return pd.DataFrame()

        # ✅ Fix the warning by using `.loc[:, "column"]`
...
for role in job_roles:
    logging.info(f"🚀 Starting job search for role: {role}")  # ✅ Added log statement
...
if not all_jobs_df.empty:
    ...
    logging.info(f"✅ Final unique jobs saved in {final_csv_filename}")
else:
...
def send_email_with_attachment(file):
    ...
    if not os.path.exists(file):
        logging.warning("⚠️ No file to send. Exiting email function.")
        return
